...
<div class="row">
	<div class="col-sm-12 col-md-12 col-lg-8 order-lg-2">
		{$PageBlock}
		<div class="row ">
			<form id="search-form" class="col-sm-8 form-inline" method="post" action="{$OffersHref}">
				<input  id="tags-id" name="tags" class="form-control" type="hidden" value="{$TagsValue}">
				<input  id="jobnames-id" name="jobnames" class="form-control" type="hidden" value="{$JobNamesValue}">
				<input  id="scores-id" name="scores" class="form-control" type="hidden" value="{$ScoresValue}">
				<input  id="prices-id" name="prices" class="form-control" type="hidden" value="{$PricesValue}">
				<input  id="countries-id" name="countries" class="form-control" type="hidden" value="{$CountriesValue}">
				<input  id="locations-id" name="locations" class="form-control" type="hidden" value="{$LocationsValue}">
				<div class="input-group">
					<input  id="search-id" name="search" class="form-control" type="text" placeholder="{#Txt_offers_search_placeholder#}" size="50" value="{$SearchValue}">
			...
			<div class="card-body">
				...
				<div class="row form-group">
					...
					<div id="prices-input" class="col-lg-10 col-sm-7 input-group">
						<input id="prices-type" class="integer form-control" type="number" step="1" style ="appearance: textfield;" pattern="\d*" min="0" max="9999" size="4" maxlength="4" value="{$PricesValue}" placeholder="{#Txt_offers_prices_placeholder#}">
						<span class="input-group-addon">{#Txt_offers_prices_addon#}</span>
					</div>
		...
		$(document).ready(
			function() {
				...
				function getJobNames() {
					$.ajax({
						url: '{/literal}{$JobNamesHref}{literal}',
						success: function(data) {
							jobnames = data;
				...
				function getTree() {
					$.ajax({
						url: '{/literal}{$LocationsHref}{literal}',
						success: function(data) {
							tree = data;
							initTree();
							var options = {
