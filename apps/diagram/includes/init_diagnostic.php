...
function analyseNodeVariable($id, $nodeId = '', $step = 0) {
	$ws = workspace::ws_open();
	$traceItem = array();
...
function analyseSlideVariable($id, $slideId = '', $step = 0) {
	$ws = workspace::ws_open();
	$traceItem = array();
	...
	if (empty($slideId)) {
		...
		if ($step == 0) {
			$variableList = $slide['variable_list'];

		}
		else {
			$variableList = $slide['variable_1_list'];
		}
	}
	for($i=0; $i < count($variableList); $i++) {
		$variable = $variableList[$i];
...
function analyseSlideLink($slideId = '') {
	$ws = workspace::ws_open();

	$linkList = array();
	$linkListDisplay = array();
	if (!empty($slideId)) {
		$objectSlide = new object_slide();
		$slide = $objectSlide->display($slideId)->returnGet();
		$linkList = $slide['link_list'];
	}
	return $linkList;
}
...
/*      Result functions        */
/********************************/
function analyseNodeResult($id, $nodeId = '', $step = 0) {
	$ws = workspace::ws_open();
	$traceItem = array();
	$resultList = array();
...
function analyseSlideResult($id, $slideId = '', $step = 0) {
	$ws = workspace::ws_open();
	$traceItem = array();
	$resultList = array();
	$resultDisplayList = array();
	...
	$slide = $objectSlide->display($slideId)->returnGet();
	if ($step == 0) {
		$resultList = $slide['result_list'];
	}
	else {
		$resultList = $slide['result_1_list'];
	}
	for($i=0; $i < count($resultList); $i++) {
		$result = $resultList[$i];
		$field = $result['name'];
		...
		if (($display == 1) and(isset($displayResult[$field]))) {
//			$display = $displayResult[$field];
			$result['display'] = $displayResult[$field];
		}
		if ($display == 1) {
			$resultDisplayList[] = $result;
		}
...
function showResult($id, $resultList, $categoryFlag=true) {
	$ws = workspace::ws_open();
	$traceItem = array();
	$return = '';
...
function showLink($id, $linkList) {
	...
	for($i=0; $i < count($linkList); $i++) {
		$slideId = $linkList[$i]['id'];
		$title = $linkList[$i]['title'];
		if ($i == 0) {
			$wcrud->fieldSet('bt_slide_' . $slideId, 'button', 'slides_box', 'id');
		}
		else {
			$wcrud->fieldLineSet('bt_slide_' . $slideId, 'button', 'slides_box', 'id');
		}
		$wcrud->textSet('bt_slide_' . $slideId, $title);
	}
	$return = $wcrud->fetchCrudEdit($id);
	return $return;
}
