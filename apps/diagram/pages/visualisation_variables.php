...
if ($domainId != '') {
	$visuJs = $ws->paramGet($ws->paramGet('APP_NAME') . '_RELA_JS_DIR') . 'visu-diagram-variable.js';
	$ws->assign('visuJs', $visuJs);

	$connect = new object_connect();
	$ws->assign('pageRef', $connect->constructHref($ws->paramGet('APP_CODE'), 'diagram_variables_visu'));
	$ws->assign('domainId', $domainId);

	$ws->logTrace($ws->paramGet('APP_CODE'), $ws->paramGet('PAGE_NAME'), $ws->paramGet('ID'), $ws->urlTitleGet());

	$ws->caching = false;
	$ws->build('visualisation_variables.tpl');
}
else {
