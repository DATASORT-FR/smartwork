...
if ($domainId != '') {
	...
	$pageSave = $connect->constructHref($ws->paramGet('APP_CODE'), 'diagram_variables_visu', 'command:update');
//	$pageSave = $connect->constructHref($ws->paramGet('APP_CODE'), 'traces', 'mode:api', 'id:' .$id);
	...
	for($i=0; $i < count($variableDisplayList); $i++) {
		...
		if (($variable['line'] == 0) and ($type != 10)) {
