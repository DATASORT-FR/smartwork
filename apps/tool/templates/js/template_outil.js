...
//			div.appendChild(content);
		...
		nodeCalculateSelect = function(code) {
			...
			diagramInfo['diagram_nodes'] = diagramInfo['diagram_nodes'] + code;
			nodeRegister = nodeFind(code);
		...
		displayVisu = function(code) {
			...
			$.ajax({
				...
				success: function(data) {
					block.find(".title:first").html(data['title']);
					block.find(".intro:first").html(data['description']);
					block.find(".content:first").html(data['description_notice']);
		...
		displayProcessVisu = function(code) {
			...
			nodeRegister = nodeFind(code);
			title = nodeRegister['title'];
			processText = nodeRegister['process_text'];
			processText2 = nodeRegister['process_text2'];
			description = nodeRegister['description'];
...
//			block.find(".header:first").html(title);
			block.find(".first:first").html(processText);
			block.find(".second:first").html(processText2);
			block.find(".image img:first").attr("src", imageNode);
			blockBody.find(".variable-link:first").remove();
			if (information == 1) {
				blockBody.append(""
